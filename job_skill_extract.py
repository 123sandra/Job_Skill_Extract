...
# Download NLTK data (run once or include in setup)
nltk.download('punkt')
...
def extract_skills(text):
    ...
    tokens = word_tokenize(text)  # Tokenize for single-word skills
    found_skills = set()
    for skill in skills_list:
        if ' ' not in skill:  # Single-word skills
            if skill in tokens:
                found_skills.add(skill)
        else:  # Multi-word skills
            if skill in text:  # Search the full text, not just tokens
                found_skills.add(skill)
